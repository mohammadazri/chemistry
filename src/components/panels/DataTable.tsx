        ...
        <div className="flex flex-col h-full text-foreground">
            <div className="flex justify-between items-center mb-6">
                <div>
                    ...
                    <p className="text-xs text-indigo-600 dark:text-indigo-300/70 font-medium uppercase tracking-wider mt-1">Target: ~{equivVol.toFixed(2)} mL</p>
                </div>
                <button
            ...
            <div className="flex-1 overflow-hidden flex flex-col border border-indigo-500/20 rounded-2xl bg-card/50 shadow-inner backdrop-blur-sm relative">
                {/* Table Header */}
                <div className="grid grid-cols-4 gap-2 px-4 py-3 border-b border-indigo-500/20 bg-indigo-500/5 text-[10px] font-bold text-indigo-600 dark:text-indigo-300 uppercase tracking-widest sticky top-0 z-10 backdrop-blur-md">
                ...
                <div ref={scrollRef} className="max-h-[300px] overflow-y-auto w-full customized-scrollbar">
                    {titrationData.length === 0 ? (
                        <div className="flex flex-col items-center justify-center h-full text-indigo-600/50 dark:text-indigo-300/50 space-y-3">
                        ...
                        <div className="flex flex-col pb-2">
                            {titrationData.map((d, i) => {
                                ...
                                return (
                                    <div
                                        key={i}
                                        className={`grid grid-cols-4 gap-2 px-4 py-2.5 border-b border-border/50 text-sm hover:bg-muted/50 transition-colors items-center
                      ${isClosest ? 'bg-indigo-500/10 dark:bg-indigo-500/20 border-l-2 border-l-indigo-400 shadow-[inset_0_0_15px_rgba(99,102,241,0.15)] relative overflow-hidden' : ''}`}
                                    >
                                        {isClosest && <div className="absolute inset-0 bg-gradient-to-r from-indigo-500/10 to-transparent pointer-events-none" />}
                                        <div className="text-center text-muted-foreground font-mono text-xs">{i + 1}</div>
                                        <div className="text-right font-mono text-muted-foreground relative z-10">{d.volume.toFixed(2)}</div>
                                        <div className="text-right font-mono font-bold text-foreground relative z-10 drop-shadow-md">{d.ph.toFixed(2)}</div>
